...
# Model-predictive control planner with cross-entropy method and learned transition model
class POP_P_MPPI(jit.ScriptModule):
  __constants__ = ['action_size', 'num_params', 'planning_horizon', 'optimisation_iters', 'candidates', 'top_candidates', 'min_action', 'max_action', 'gamma', 'beta']

  def __init__(self, action_size, planning_horizon, optimisation_iters, candidates, top_candidates, transition_model, reward_model, policy_net, min_action=-inf, max_action=inf, initial_sigma=1, gamma=10, beta=0.6):
    super().__init__()
    self.transition_model, self.reward_model = transition_model, reward_model
    self.policy_net = policy_net
    self.action_size, self.min_action, self.max_action = action_size, min_action, max_action
    ...
    self.gamma = gamma
    self.beta = beta
    self.num_params = self.policy_net.num_units
  ...
  def forward(self, belief, state):
    ...
    num_params = self.policy_net.num_units
    # Initialize factorized belief over action sequences q(a_t:t+H) ~ N(0, I)
    init_mean, init_std_dev = torch.zeros(self.planning_horizon, B, 1, num_params, device=belief.device), torch.ones(self.planning_horizon, B, 1, num_params, device=belief.device)* self.initial_sigma
    mean = torch.zeros(self.planning_horizon, B, 1, num_params, device=belief.device)

    for _ in range(self.optimisation_iters):
      # Evaluate J action sequences from the current belief (over entire sequence at once, batched over particles)
      
      uu = (init_mean + init_std_dev * torch.randn(self.planning_horizon, B, self.candidates, num_params, device=init_mean.device))#.view(self.planning_horizon, B * self.candidates, self.action_size)  # Sample actions (time x (batch x candidates) x actions)
      N = torch.zeros(self.planning_horizon, B, self.candidates, num_params, device=belief.device)
      ...
      noises = (mean + N).view(self.planning_horizon, B*self.candidates, num_params)
      ...
      den = returns.sum(1) + 1e-8
      returns = returns.reshape(1, B, self.candidates, 1)
      actions = noises.view(self.planning_horizon, B, self.candidates, self.num_params)
      weighted_actions = returns * actions
      den = den.reshape(1, len(den), 1, 1)
      mean = (weighted_actions/den).sum(2).unsqueeze(2)

    actions = self.policy_net(ibelief, istate, mean[0].squeeze(dim=1))
    actions.clamp_(min=self.min_action, max=self.max_action)  # Clip action range
    return actions
